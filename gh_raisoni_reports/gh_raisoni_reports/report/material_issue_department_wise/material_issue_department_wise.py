...
def get_columns():
    return [
        ...
        },
        {
            "label": "User Department",
            "fieldname": "custom_department",
            "fieldtype": "Data",   # ✅ sirf display ke liye
            "width": 200
        },
        {
            "label": "Item Code",
...
def get_data(filters):
    ...
    if filters.get("custom_department"):
        conditions += """
        AND TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(se.custom_username, ' - ', 2), ' - ', -1)) = %(custom_department)s
        """
    ...
    data = frappe.db.sql("""
        SELECT
            ...
            CONCAT(
                TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(se.custom_username, ' - ', 2), ' - ', -1)),
                ' - ',
                TRIM(SUBSTRING_INDEX(se.custom_username, ' - ', -1))
