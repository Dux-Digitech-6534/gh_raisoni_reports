...
frappe.query_reports["Purchase Receipt Report New"] = {
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (column.fieldname === "purchase_invoice_details" && data && data.purchase_invoice_details && data.purchase_invoice_details !== "-") {
			var pi_list = String(data.purchase_invoice_details).split(",").map(function (pi) {
				return pi.trim();
			}).filter(Boolean);

			return pi_list.map(function (pi) {
				return '<a href="/app/purchase-invoice/' + encodeURIComponent(pi) + '" target="_blank">' +
					pr_esc(pi) +
				'</a>';
			}).join(", ");
		}
		...
		return '' +
			...
			'>' +
				...
				'<td>' +
					...
					'</a>' +
				'</td>' +
				'<td title="' + pr_esc(supplier) + '">' + pr_esc(supplier || "-") + '</td>' +
				'<td>' + pr_make_link("purchase-order", purchase_order) + '</td>' +
